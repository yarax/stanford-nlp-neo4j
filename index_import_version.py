...
def check_class_relations(words):
	for i, word in enumerate(words):
		...
		if (lemma == "be"):
			child = words[i-1]
			parent = words[i+1]
			if (parent[1]["PartOfSpeech"] == "DT"):
				parent = words[i+2]
			q = "MATCH (a:{0}), (b:{1}) MERGE a-[rel:{2}]->b".format(child[0], parent[0], RELATION_TYPES["class"])
			#print q
			gdb.query(q = q)
		if (lemma != origin):
...
corenlp = StanfordCoreNLP('./stanford_corenlp_python/stanford-corenlp-full-2014-08-27/')
result = loads(corenlp.parse(text))
...
if (context == "ignore"):
	...
	check_class_relations(sentence["words"])
